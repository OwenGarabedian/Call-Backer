    ...
    <div className="relative w-full bg-[#FAFAFA] flex flex-col items-center pt-28 pb-16 px-4 overflow-hidden lg:hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center relative z-10 w-full mb-12">
        ...
        <div className="flex flex-col gap-3 w-full max-w-xs mx-auto">
          <MagneticButton variant="primary" size="lg" className="w-full" onClick={() => navigate("/sign-up")}>Start Capturing Leads</MagneticButton>
          <MagneticButton variant="secondary" size="lg" className="w-full" onClick={() => navigate("/Pricing")}>View Pricing</MagneticButton>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} className="text-center z-10 w-full mb-12 relative">
        ...
        <div className="w-full max-w-xs mx-auto flex flex-col items-center gap-4">
          <MagneticButton variant="primary" size="lg" className="w-full bg-[#111] text-white overflow-hidden relative group p-3" onClick={() => navigate("/sign-up")}>
            <span className="relative z-10 flex items-center justify-center gap-2">
              Start Capturing Leads <Smartphone className="w-4 h-4 text-purple-400" />
            </span>
          </MagneticButton>
...
// --- 3. DESKTOP SIMPLE LAYOUT ---
const DesktopHero = () => {
  const navigate = useNavigate();

  return (
    <div className="hidden lg:block relative bg-[#FAFAFA] font-sans selection:bg-[#1A1A1A] selection:text-white">
      {/* Backgrounds */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[70vw] h-[70vw] rounded-full bg-gradient-to-br from-[#E0CCF7] to-transparent opacity-40 blur-3xl" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[60vw] h-[60vw] rounded-full bg-gradient-to-tl from-[#D4E4F7] to-transparent opacity-30 blur-3xl" />
        <div className="absolute inset-0 opacity-[0.03] bg-[url('https://grainy-gradients.vercel.app/noise.svg')]" />
      </div>

      {/* Hero Entry — text left, phone right */}
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 grid grid-cols-2 items-center pt-36 pb-24 gap-12">
        <motion.div
        ...
        >
          ...
          <p className="text-lg text-gray-500 max-w-md mb-10 leading-relaxed font-light">
            You're busy on the job site. We're busy on the phone.{" "}
            <strong className="text-gray-900 font-semibold block mt-1">Capture, qualify, and convert leads automatically.</strong>
          </p>
        ...
        <div className="flex items-center justify-center">
          <Suspense fallback={null}>
            <PhoneMockup scrollUp={null} stopFloating={false} isScrolling={false} />
          </Suspense>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          >
            <PoshCard>
              <div className="flex flex-col h-full justify-between">
                <div className="flex justify-between items-start">
                  <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100"><Zap className="w-5 h-5 text-gray-900" /></div>
                  <span className="text-[10px] font-mono uppercase tracking-wider text-gray-400 border border-gray-100 px-2 py-1 rounded-full">Speed</span>
                </div>
                <div>
                  <h3 className="text-2xl font-medium text-gray-900 tracking-tight mb-2">Zero Delay</h3>
                  <p className="text-gray-500 text-sm">Customers appreciate speed. Our AI responds in 2.5s.</p>
                </div>
              </div>
            </PoshCard>
          </motion.div>
          ...
          >
            <PoshCard>
              <div className="flex flex-col h-full justify-between">
                <div className="flex justify-between items-start">
                  <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100"><Clock className="w-5 h-5 text-gray-900" /></div>
                  <span className="text-[10px] font-mono uppercase tracking-wider text-gray-400 border border-gray-100 px-2 py-1 rounded-full">Reliability</span>
                </div>
                <div>
                  <h3 className="text-2xl font-medium text-gray-900 tracking-tight mb-2">Always Open</h3>
                  <p className="text-gray-500 text-sm">Whether you're on a roof or at dinner, we ensure response.</p>
                </div>
              </div>
            </PoshCard>
          </motion.div>
          ...
          >
            <PoshCard className="relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-blue-100 to-transparent rounded-full opacity-50 translate-x-1/3 -translate-y-1/3 pointer-events-none" />
              <div className="relative z-10 flex flex-row items-end justify-between gap-8 h-full">
                <div className="max-w-md">
                  <div className="flex items-center gap-3 mb-6">
                    <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100"><TrendingUp className="w-5 h-5 text-gray-900" /></div>
                    <div className="flex items-center gap-1 px-2 py-1 bg-green-50 rounded-full border border-green-100">
                      <CheckCircle2 className="w-3 h-3 text-green-600" />
                      <span className="text-[10px] font-bold text-green-700 uppercase tracking-wide">Optimization</span>
                    </div>
                  </div>
                  <h3 className="text-3xl font-medium text-gray-900 tracking-tight mb-3">Grow Revenue</h3>
                  <p className="text-gray-500 text-base">Stop leaving money on the table. We automatically re-engage missed calls to book more jobs without extra work.</p>
                </div>
                <div className="bg-white/50 border border-white/60 rounded-2xl p-6">
                  <div className="flex items-end gap-2">
                    <span className="text-5xl font-light text-gray-900 tracking-tighter">30%</span>
                    <ArrowUpRight className="w-8 h-8 text-emerald-500 mb-2" />
                  </div>
      ...
      <motion.div
        ...
        className="relative z-10 text-center max-w-2xl mx-auto px-6 pb-16"
      >
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          >
            <span className="relative z-10 flex items-center justify-center gap-3 text-white">
              <span className="text-lg font-medium tracking-wide">Start Capturing Leads</span>
...
//                             <span className="relative z-10 flex items-center justify-center gap-3 text-white">
//                                 <span className="text-lg font-medium tracking-wide">Start Capturing Leads</span><Smartphone className="w-5 h-5 text-purple-400 group-hover:text-white group-hover:translate-x-1 group-hover:-translate-y-1 transition-all duration-300" strokeWidth={2.5} />
//                             </span>
//                             <div className="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/5 to-white/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
//                         </MagneticButton>
//                     </Link>
